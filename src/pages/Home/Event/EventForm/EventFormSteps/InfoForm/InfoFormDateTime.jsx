...
export const InfoFormDateTime = observer(() => {
    ...
    useEffect(() => {
        const fromDate = dayjs(`${fromDay}-${fromMonth}-${fromYear} ${fromHour}:${fromMin}`, "D-M-YYYY H:m")
        const untilDate = dayjs(`${untilDay}-${untilMonth}-${untilYear} ${untilHour}:${untilMin}`, "D-M-YYYY H:m")
        if (fromDate.isValid() && untilDate.isValid() && fromDate.valueOf() >= untilDate.valueOf()) {
    ...
    }, [fromDay, fromMonth, fromYear, fromHour, fromMin, untilDay, untilMonth, untilYear, untilHour, untilMin])
            ...
            <div className="infoform__element">
                <Row gutter={[8, 8]}>
                    <Col xs={24} sm={24} md={11}>
                        ...
                        <Space.Compact style={{ width: 'calc(35% - 8px)', paddingLeft: '8px' }}>
                            <InputNumber
                                style={{ width: '50%' }}
                                placeholder='hour'
                                value={fromHour}
